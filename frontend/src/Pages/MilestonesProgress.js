...
const MilestonesProgress = () => {
  ...
  const testRuns = [
    { id: "201", name: "Functional Test", date: "2024-08-01", author: "Alice" },
    { id: "202", name: "Integration Test", date: "2024-08-02", author: "Bob" },
    { id: "203", name: "Performance Test", date: "2024-08-03", author: "Charlie" },
  ...
  const handleEditMilestone = () => {
    navigate(
      `/add-milestone?milestoneId=${milestoneId}&milestoneName=${milestoneName}`,
      {
        state: {
  ...
  const handleTestRunClick = (testRunId, testRunName) => {
    navigate(
      `/TestRunTestsResults?milestoneId=${milestoneId}&milestoneName=${milestoneName}&testRunId=${testRunId}&testRunName=${testRunName}`
    );
  };
      ...
      <div className="progress-header">
        <div className="activity-options">
          ...
          sourcePage === "milestone-defect" ? (
            ...
            <>
              <a
                href={`/milestone-status?milestoneId=${milestoneId}&milestoneName=${milestoneName}`}
              >
                Status
              </a>
              <a
                href={`/milestone-activity?milestoneId=${milestoneId}&milestoneName=${milestoneName}&source=milestone-status`}
              >
                Activity
              </a>
              <a
                className="upperbar"
                href={`/milestone-progress?milestoneId=${milestoneId}&milestoneName=${milestoneName}&source=milestone-status`}
              >
                Progress
              </a>
              <a
                href={`/milestone-defect?milestoneId=${milestoneId}&milestoneName=${milestoneName}&source=milestone-status`}
              >
                Defects
              </a>
            </>
        ...
        <div className="progress-controls">
          ...
          <div className="progress-control-button">
            <button className="milestone-button" onClick={handleEditMilestone}>
              Edit
            </button>
          </div>
        </div>
      ...
      {sourcePage !== 'TestRunTestsResults' && sourcePage !== 'milestone-activity' && sourcePage !== 'milestone-progress' && sourcePage !== 'milestone-defect' && (
            ...
            <div className="progress-testrun-class">
              {testRuns.map((testRun) => (
                <div
                  key={testRun.id}
                  className="progress-testrun-details"
                >
                  <p>
                    <a
                  ...
                  <div className="progress-testrun-forecast">
                    <p>Forecast for {testRun.name}</p>
                    <p>Author: {testRun.author}</p>
                    <p>Date: {testRun.date}</p>
                  </div>
                </div>
              ))}
            </div>
